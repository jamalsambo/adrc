...
async function handleSubmit() {
  try {
    if (isEdit.value) {
      await customerStore.update(route.params.id, {
        ...form,
      ...
      notifySuccess("Cliente registado com sucesso");
    }
  } catch (error) {
...
onMounted(async () => {
  if (route.params.id) {
    isEdit.value = true;
    await customerStore.findOne(route.params.id);
    Object.assign(form, customerStore.customer);
  }
});
</script>
